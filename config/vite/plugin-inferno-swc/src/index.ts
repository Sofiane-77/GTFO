...
type ResolvedSwcPlugin = [string, Record<string, unknown>]
...
const inferno = (_options?: Options): Plugin[] => {
  ...
  const normalized: NormalizedOptions = {
    ...
    plugins: _options?.plugins?.map((entry) => [resolve(entry[0]), entry[1]]) as
      | ResolvedSwcPlugin[]
      | undefined,
  ...
  let resolvedBuildTarget: JscTarget | undefined = normalized.buildTarget
  ...
  const mainPlugin: Plugin = {
    ...
    configResolved(resolved) {
      ...
      if (!resolvedBuildTarget) {
        resolvedBuildTarget = pickBuildTarget(resolved)
      }
    },
    async transform(code, id, transformOptions) {
      ...
      const target =
        command === 'build' || isServer
          ? resolvedBuildTarget ?? 'esnext'
          : normalized.devTarget
...
const pickBuildTarget = (config: ResolvedConfig): JscTarget => {
  ...
  return value as JscTarget
}

const enhanceUserConfig = (config: UserConfig, options: NormalizedOptions) => {
  config.esbuild ??= {}
  if (config.esbuild.jsx == null) {
    config.esbuild.jsx = 'preserve'
  }
  if (options.runtime === 'classic') {
    if (config.esbuild.jsxFactory == null) {
      config.esbuild.jsxFactory = options.jsxFactory
    }
    if (config.esbuild.jsxFragment == null) {
      config.esbuild.jsxFragment = options.jsxFragment
    }
  } else if (config.esbuild.jsxImportSource == null) {
    config.esbuild.jsxImportSource = options.importSource
  }

  config.optimizeDeps ??= {}
  const include = (config.optimizeDeps.include ||= [])
  for (const dep of ['inferno', 'inferno-create-element']) {
...
const transformWithOptions = async (
  ...
  try {
    ...
    const err = e as { message?: string; line?: string; column?: string }
